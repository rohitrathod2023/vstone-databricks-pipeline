...
DIM_DATE_SCHEMA = """
    ...
    day_of_week     INT     COMMENT 'Spark dayofweek() convention: 1=Sunday through 7=Saturday -- NOT ISO-8601 (1=Monday).',
    day_name        STRING  COMMENT 'Full day name, e.g. Monday.',
    quarter         INT     COMMENT 'Calendar quarter, 1-4.',
    is_weekend      BOOLEAN COMMENT 'True for Saturday/Sunday, using the same day_of_week convention (Sunday=1 or Saturday=7).',
    load_dt         TIMESTAMP COMMENT 'Timestamp this row was loaded.',
    source_format   STRING  COMMENT 'Always "generated" -- Dim_Date has no source file, it is derived from the observed min/max date across Silver.',
...
DIM_STREET_SCHEMA = """
    street_key      INT     NOT NULL COMMENT 'Surrogate key -- unique per SCD2 version, NOT per street (a street with 2 historical versions has 2 street_key values).',
    ...
    dangerous       DOUBLE  COMMENT 'Danger rating for this street version, valid range 0.1-0.9 (higher = more dangerous). SCD2-tracked: a change here creates a new street_key version.',
    __START_AT      TIMESTAMP COMMENT 'SCD2: when this version of the row became active.',
    __END_AT        TIMESTAMP COMMENT 'SCD2: when this version of the row stopped being active. NULL for the current version.',
...
FACT_CITY_OBSERVATIONS_SCHEMA = """
    street_key          INT     COMMENT 'FK to dim_street. NULL for rows that are not street-environment observations.',
    location_key        INT     COMMENT 'FK to dim_location. NULL for rows that are not traffic observations, or where the traffic reading has no matching location (see location=7 orphan case).',
    date_key            INT     NOT NULL COMMENT 'FK to dim_date.',
    time_key            INT     NOT NULL COMMENT 'FK to dim_time.',
    technique_key        INT     NOT NULL COMMENT 'FK to dim_technique -- which of the 4 Bronze ingestion techniques this observation came from.',
    audit_key           INT     NOT NULL COMMENT 'FK to dim_audit.',
    noise               DOUBLE  COMMENT 'Environmental branch measure: noise level reading. NULL for non-environmental rows.',
    pollution           DOUBLE  COMMENT 'Environmental branch measure: pollution level reading. NULL for non-environmental rows.',
    light               DOUBLE  COMMENT 'Environmental branch measure: light level reading. NULL for non-environmental rows.',
    raining             DOUBLE  COMMENT 'Environmental branch measure: rain intensity reading, valid range 0-100. NULL for non-environmental rows.',
    enter               INT     COMMENT 'Traffic branch measure: vehicles entering the intersection. NULL for non-traffic rows.',
    exit                INT     COMMENT 'Traffic branch measure: vehicles exiting the intersection. NULL for non-traffic rows.',
    vehicle_plate_id    INT     COMMENT 'Traffic branch measure: traffic reading sequence number (originally cars.csv id). NULL for non-traffic rows.',
    message_count       INT     COMMENT 'Telegram branch measure: number of citizen reports in this observation. NULL for non-telegram rows.',
...
AGG_DAILY_STREET_CONDITIONS_SCHEMA = """
    street_key                  INT     NOT NULL COMMENT 'FK to dim_street, part of this table''s composite grain.',
    street_id                   INT     COMMENT 'Natural key, denormalized from dim_street for convenience.',
    street                      STRING  COMMENT 'Street name, denormalized from dim_street for convenience.',
    date_key                    INT     NOT NULL COMMENT 'FK to dim_date, part of this table''s composite grain.',
    full_date                   DATE    COMMENT 'Calendar date, denormalized from dim_date for convenience.',
    ...
    observation_count           BIGINT  COMMENT 'Number of raw environmental observations rolled up into this row.',
    load_dt                     TIMESTAMP COMMENT 'Timestamp this row was computed.',
    source_format               STRING  COMMENT 'Always "aggregated" -- this is a Gold rollup, not a direct source load.',
...
AGG_DAILY_LOCATION_TRAFFIC_SCHEMA = """
    location_key                INT     COMMENT 'FK to dim_location. NULL for the location=7 orphan group (bad coordinates, excluded from Dim_Location at Silver).',
    location                    INT     COMMENT 'Natural key, denormalized from dim_location for convenience.',
    ...
    observation_count           BIGINT  COMMENT 'Number of raw traffic observations rolled up into this row.',
    load_dt                     TIMESTAMP COMMENT 'Timestamp this row was computed.',
    source_format               STRING  COMMENT 'Always "aggregated" -- this is a Gold rollup, not a direct source load.',
    source_file                 STRING  COMMENT 'Identifies this as a derived aggregate, not a source file.',
    run_id                      STRING  COMMENT 'Identifier for the pipeline run that computed this row.',
...
AGG_MONTHLY_STREET_SUMMARY_SCHEMA = """
    ...
    street_id                   INT     COMMENT 'Natural key, denormalized from dim_street for convenience.',
    street                      STRING  COMMENT 'Street name, denormalized from dim_street for convenience.',
    dangerous                   DOUBLE  COMMENT 'Danger rating for this street, denormalized from dim_street (current version at aggregation time).',
    year                        INT     NOT NULL COMMENT 'Calendar year, part of this table''s composite grain.',
    month                       INT     NOT NULL COMMENT 'Calendar month, part of this table''s composite grain.',
    ...
    days_with_rain               BIGINT  COMMENT 'Count of distinct days in this month with a nonzero rain intensity reading for this street.',
    observation_count           BIGINT  COMMENT 'Number of raw environmental observations rolled up into this row.',
    observation_days            BIGINT  COMMENT 'Number of distinct days with at least one observation for this street in this month.',
    load_dt                     TIMESTAMP COMMENT 'Timestamp this row was computed.',
    source_format               STRING  COMMENT 'Always "aggregated" -- this is a Gold rollup, not a direct source load.',
    source_file                 STRING  COMMENT 'Identifies this as a derived aggregate, not a source file.',
    run_id                      STRING  COMMENT 'Identifier for the pipeline run that computed this row.',
...
AGG_HOURLY_TELEGRAM_ACTIVITY_SCHEMA = """
    ...
    day_name                    STRING  COMMENT 'Day name, denormalized from dim_date.',
    is_weekend                  BOOLEAN COMMENT 'Weekend flag, denormalized from dim_date.',
    hour                        INT     NOT NULL COMMENT 'Hour of day, 0-23, part of this table''s composite grain. Not a FK to dim_time -- dim_time''s PK is (time_key, HHMMSS), not bare hour.',
    total_messages               BIGINT  COMMENT 'Total citizen reports received in this date/hour.',
    observation_count           BIGINT  COMMENT 'Number of raw telegram observations rolled up into this row.',
    load_dt                     TIMESTAMP COMMENT 'Timestamp this row was computed.',
    source_format               STRING  COMMENT 'Always "aggregated" -- this is a Gold rollup, not a direct source load.',
    source_file                 STRING  COMMENT 'Identifies this as a derived aggregate, not a source file.',
    run_id                      STRING  COMMENT 'Identifier for the pipeline run that computed this row.',
